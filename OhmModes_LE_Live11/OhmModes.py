...
class OhmModes(LividControlSurface):
	...
	def _initialize_script(self):
		...
		self._main_modes.selected_mode = 'LE'
	...
	def _setup_session_control(self):
		...
		self._session_navigation.layer = Layer(priority = 5, up_button = self._menu[0], down_button = self._menu[3], left_button = self._menu[4], right_button = self._menu[5])
		...
		self._session.layer = Layer(priority = 5, clip_launch_buttons = self._matrix.submatrix[:7,:7],
	...
	def _setup_mixer_control(self):
		...
		self._mixer.layer = Layer(priority = 5, volume_controls = self._fader_matrix.submatrix[:7, :], crossfader_control = self._crossfader)
	...
	def _setup_transport_control(self):
		...
		self._transport.layer = Layer(priority = 5, tap_tempo_button = self._livid)
	...
	def _setup_modes(self):
		...
		# 									self._mixer.instrument_layer],
		# 									behaviour = DefaultedBehaviour(default_mode = 'Mix'))
		self._main_modes.add_mode('LE', [self._mixer,
											self._session,
											self._session_navigation,
											self._transport])
