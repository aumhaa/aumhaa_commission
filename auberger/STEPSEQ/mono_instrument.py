...
ALSO:  apparently _c_instance.playhead can only deal with 32 notes, so anything beyond that will get ignored.  Fucking great, abes.  Wasted over 8 hours on that.
"""

class SpecialPlayheadComponent(PlayheadComponent):

	def __init__(self, *a, **k):
		self._playhead_page = 0
		self._note_banks = []
	...
	def set_note_banks(self, note_banks):
		...
		self._note_bank_length = len(note_banks[0])
	...
	def update(self):
		...
		if self._playhead:
			...
			if clip:
				...
				#page_offset = self._follower._paginator.page_index
				playhead_offset = self._playhead_page % 4
				notes = self._triplet_note_banks[playhead_offset] if is_triplet else self._note_banks[playhead_offset]
				self._playhead.notes = list(notes)
				wrap_around = self._follower.is_following and self._paginator.can_change_page
				self._playhead.wrap_around = wrap_around
	...
	def set_follower(self, follower):
		...
		self._on_follower_is_following_changed.subject = follower
		#self._on_follower_page_index_changed.subject = follower
		#self._on_follower
...
class MonoStepSeqComponent(StepSeqComponent):
	...
	def __init__(self, *a, **k):
		...
		self._playhead_component = SpecialPlayheadComponent(parent=self, grid_resolution=self._grid_resolution, paginator=self.paginator, follower=self._loop_selector, notes=chain(*starmap(range, ((92, 100),
		 (84, 92),
		 (76, 84),
...
class MonoInstrumentComponent(Component):
	...
	# def _on_keypad_page_index_changed(self, value):
	# 	debug('keypad playhead_page_index', value)
	#
	# @listens(u'playhead_page_index')
	# def _on_drumpad_page_index_changed(self, value):
	# 	debug('drumpad playhead_page_index', value)
