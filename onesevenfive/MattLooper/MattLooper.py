...
from ableton.v2.control_surface import ControlSurface, Layer, Skin, PrioritizedResource
...
LIVE_STATE_COLORS = [2, 5, 6, 3]
FLASH_DELAY = 4
LOOP_STATUS_LEDS = [12, 16, 13, 14, 15]
COLOR_MAP = [127, 24, 66, 97, 124, 49, 82]
...
class MonolooperComponent(Component):
	...
	def __init__(self, leds, script, *a, **k):
		...
		self._loopers = [Monoloop(self, index, self._leds[index]) for index in range(5)]
	...
	def _select_next_loop(self):
		...
		self._select_looper(min(4, self._selected_loop._index + 1))
	...
	def on_livelooper_state_change(self):
		if self.is_enabled():
			if not self.on_livelooper_state_change.subject == None:
				# debug('on State changed:', self.on_livelooper_state_change.subject.value)
...
class MattLooper(ControlSurface):
	...
	def _setup_controls(self):
		...
		self._leds = [None for index in range(5)]
		for index in range(5):
				...
				self._leds[index] =  MonoButtonElement(is_momentary = True,
					msg_type = MIDI_NOTE_TYPE,
					channel = 1,
					identifier = LOOP_STATUS_LEDS[index],
					name = 'LED_' + str(index),
					color_map = COLOR_MAP,
	...
	def update_display(self):
		super(MattLooper, self).update_display()
